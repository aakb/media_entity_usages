 ...
 *
 * @FieldFormatter (
 *   id = "field_media_usage_formatter",
 *   label = @Translation("Media usage formatter"),
 *   field_types = {
 *     "field_media_usage"
 *   }
 * )
 */
class MediaUsageFormatter extends FormatterBase {
  /**
   * {@inheritdoc}
   */
  public function viewElements(FieldItemListInterface $items, $langcode = NULL) {
    ...
    foreach ($items as $delta => $item) {
      if (isset($item->entity_id) && isset($item->entity_type)) {
        ...
        $markup = '<a href="/' . $item->entity_type . '/' . $item->entity_id . '">' . $entity->getTitle() . '</a>';
      }
      else {
        $markup = 'No reference';
      }
